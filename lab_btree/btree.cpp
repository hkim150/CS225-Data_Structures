...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(!subroot->elements.empty() && subroot->elements.size() > first_larger_idx){
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf){subroot->elements.insert(first_larger_idx + subroot->elements.begin(), pair);}
    else{
	BTreeNode* node = subroot->children[first_larger_idx];
	insert(node, pair);
	if(order <= node->elements.size()){split_child(subroot, first_larger_idx);}
    }
}
