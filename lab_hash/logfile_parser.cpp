...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
	...
	std::string k = ll.url + ll.customer;

	if(whenVisitedTable.keyExists(k) && whenVisitedTable[k] < ll.date){whenVisitedTable[k] = ll.date;}
	else{whenVisitedTable.insert(k, ll.date);}
    }
                                  ...
                                  const string& url) const
{
     ...
     */
    std::string k = url + customer;
    if(whenVisitedTable.keyExists(k)){return whenVisitedTable.find(k);}
