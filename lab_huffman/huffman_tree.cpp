                            ...
                            queue<TreeNode*>& mergeQueue)
{
	...
	if(!mergeQueue.empty()){
	    if(mergeQueue.front()->freq.getFrequency() < smallest->freq.getFrequency()){
		smallest = mergeQueue.front();
		mergeQueue.pop();
		return smallest;
	    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	TreeNode* subroot = new TreeNode(smallest->freq.getFrequency() + secondSmallest->freq.getFrequency());
	subroot->left = smallest;
	subroot->right = secondSmallest;
	mergeQueue.push(subroot);
    }

    if(singleQueue.empty()){root = mergeQueue.front();}
    else{root = singleQueue.front();}
}
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()){
	if(bfile.getNextBit() == 1){retNode = new TreeNode(Frequency(bfile.getNextByte(), 0));}
	else{
	    retNode = new TreeNode(0);
	    retNode->left = readTree(bfile);
	    retNode->right = readTree(bfile);
	}
    }

    return retNode; // replaceme!
}
