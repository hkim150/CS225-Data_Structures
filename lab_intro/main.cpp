...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
        for (size_t xi = 0; xi < original.width(); xi++) {
            r = original(xi,yi)->red;
	    g = original(xi,yi)->green;
	    b = original(xi,yi)->blue;
	    if(r + amount > 255){r = 255;}
	    else{r += amount;}
	    if(g + amount > 255){g = 255;}
	    else{g += amount;}
	    if(b + amount > 255){b = 255;}
	    else{b += amount;}
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
	...
	for(size_t xi=0; xi<firstImage.width(); xi++){
	    R1 = firstImage(xi,yi)->red;
	    G1 = firstImage(xi,yi)->green;
	    B1 = firstImage(xi,yi)->blue;
	    R2 = secondImage(xi,yi)->red;
	    G2 = secondImage(xi,yi)->green;
	    B2 = secondImage(xi,yi)->blue;
	    firstImage(xi,yi)->red = (R1+R2)/2;
	    firstImage(xi,yi)->green = (G1+G2)/2;
	    firstImage(xi,yi)->blue = (B1+B2)/2;
	}
    }
