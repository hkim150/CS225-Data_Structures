...
class BinaryTree
{
  ...
  private:
    ...
    T maxLeftNode(const Node* subRoot) const;
    T minRightNode(const Node* subRoot) const;

    bool isOrdered(const Node* subRoot) const;

    void printPaths(std::vector<const Node*>& treeVector, const Node* subRoot) const;

    void printVector(std::vector<const Node*> treeVector) const;

    int& sumDistances(const Node* subRoot, int portion, int& sum) const;
